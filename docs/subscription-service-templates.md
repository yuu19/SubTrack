...
サブスク登録時に代表的なサービスを選択すると、サービス名、標準カテゴリー、候補価格、支払い周期、解約URL、解約方法、解約メモをフォームへ自動入力する。
...
- カテゴリーは専用の `categoryId` に自動選択する。タグ入力は廃止する。
...
- テンプレート表示名、プラン名、カテゴリー名、解約メモは日本語・英語の両方を持つ。保存後は選択時の表示言語の文字列を保持する。
...
3. 候補を選ぶ。
4. プラン候補を選ぶ。
5. サービス名、カテゴリー、周期、料金、解約URL、解約方法、解約メモをフォームへ反映する。
...
初期テンプレートは20件とする。
カテゴリーはテンプレート一覧の絞り込み用に `ServiceTemplateCategory` として保持する。
登録フォームでは、同じキーを持つ標準カテゴリーが存在する場合だけ自動選択する。
無料プランでは標準カテゴリーのみ利用できる。
カスタムカテゴリーの追加は Premium で利用できる。
現在使う標準カテゴリーは `video / music / ai / tools / storage / development / design / business / shopping` とする。
...
| サービス             | カテゴリー           | 価格候補                            | 解約URL方針                   |
| -------------------- | -------------------- | ----------------------------------- | ----------------------------- |
| Netflix              | 動画 / Video         | 公式ページで確認できる範囲のみ      | Netflixアカウント管理         |
| YouTube Premium      | 動画 / Video         | 現時点では価格なし                  | YouTube有料メンバーシップ管理 |
| Amazon Prime         | 買い物 / Shopping    | 現時点では価格なし                  | Amazon Prime会員情報管理      |
...
| iCloud+              | ストレージ / Storage | Apple公式サポートで確認できるプラン | Appleサブスクリプション管理   |
| Google One           | ストレージ / Storage | 現時点では価格なし                  | Google One設定                |
| Notion               | ツール / Tools       | 公式ページで確認できるプラン        | Notionワークスペース設定      |
| Figma                | デザイン / Design    | 公式ページで確認できる範囲のみ      | Figma管理画面                 |
| Adobe Creative Cloud | デザイン / Design    | 現時点では価格なし                  | Adobeアカウント管理           |
| Canva                | デザイン / Design    | 現時点では価格なし                  | Canva請求設定                 |
| Dropbox              | ストレージ / Storage | 公式ページで確認できるプラン        | Dropbox請求設定               |
| Microsoft 365        | ツール / Tools       | 公式ページで確認できるプラン        | Microsoftアカウント管理       |
| Google Workspace     | ビジネス / Business  | 公式ページで確認できるプラン        | Google管理コンソール請求      |
| Slack                | ビジネス / Business  | 公式ページで確認できるプラン        | Slackワークスペース請求       |
| Zoom                 | ビジネス / Business  | 現時点では価格なし                  | Zoom請求設定                  |
| GitHub               | 開発 / Development   | 公式ページで確認できるプラン        | GitHub請求設定                |
