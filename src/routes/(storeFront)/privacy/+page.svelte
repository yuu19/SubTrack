<script lang="ts">
	...
	import { privacyDocuments, privacyPageCopy } from '$lib/content/site-content';
	import { resolveLocale } from '$lib/locale';
	import { getLocalizedSeoLinks } from '$lib/locale-routing';
	import { getLocale } from '$lib/paraglide/runtime';

	const locale = $derived(resolveLocale(getLocale()));
	const copy = $derived(privacyPageCopy[locale]);
	const document = $derived(privacyDocuments[locale]);
	const seoLinks = $derived(getLocalizedSeoLinks('/privacy', locale));
</script>
