<script lang="ts">
	...
	const links = $derived([
		{ href: localizedHref('/faq'), label: copy.faq },
		{ href: localizedHref('/commercial-transactions'), label: copy.commerce },
		{ href: localizedHref('/terms'), label: m.legal_terms() },
		{ href: localizedHref('/privacy'), label: m.legal_privacy() }
	]);
</script>

<footer class="bg-muted/30 border-t">
	<div
		class="mx-auto flex max-w-5xl flex-col gap-3 px-4 py-6 text-sm md:flex-row md:items-center md:justify-between"
	>
		<div class="space-y-1">
